...
describe('generateSlots', () => {
  ...
  test('breaks removed', () => {
    ...
    expect(slots.some((s) => s.label.startsWith('10:00'))).toBe(false);
  });
  ...
  test('exception custom hours overrides', () => {
    ...
    expect(slots[0].label.startsWith('11:00')).toBe(true);
  });
  ...
  test('duration blocking + overlap exclusion with existing booking', () => {
    ...
    expect(slots.some((s) => s.label.startsWith('10:30'))).toBe(false);
    expect(slots.some((s) => s.label.startsWith('10:00'))).toBe(false);
  });
});
