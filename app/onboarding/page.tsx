...
export default function OnboardingPage() {
  const {
    ...
    defaultValues: { timezone: 'Asia/Colombo', system_currency: 'USD' },
  });
  ...
  const onSubmit = async (values: FormData) => {
    ...
    const { error: profileError } = await supabase.from('profiles').upsert({ id: user.user.id });
  ...
  return (
    <main className="mx-auto grid min-h-screen max-w-xl place-items-center p-4">
      <GlassCard className="w-full">
        ...
        <form className="mt-4 space-y-3" onSubmit={handleSubmit(onSubmit)}>
          ...
          <div>
            <Label>Slug</Label>
            <Input {...register('slug')} />
            {errors.slug ? <p className="text-xs text-red-300">{errors.slug.message}</p> : null}
          </div>
          ...
          <div>
            ...
            <Select value={currency} onValueChange={(value) => setValue('system_currency', value)}>
              <SelectTrigger>
